...
def calculate_subtotal(order):
    ...
    ### WRITE SOLUTION HERE
    # assuming and order is just an independent list, compare it with menu. if match, add menu's corresponding price
#     for k,v in menu.items():
#         if order[k]-1 == v['name']:
#             sum += v['price']
#     # print(v['price']) # acesses the price"""  """
#     # print(v['name']) # acesses the order item
#     print(calculate_subtotal(order))
#    # return sum
    
    # assumimg an order is dictionary with key ('espresso')-val('1.99')
    # order = {'espresso': 1.99, 'coffee': 2.50, 'cake': 2.79, 'soup': 4.50, 'sandwich': 4.99} 
    ...
    for k in order:
        sum +=k['price']
    # print('price', round(sum,2)) # accesses the price
    return round(sum,2)
...
def calculate_tax(subtotal):
    ...
    # for k in order:
    #     # sum +=k['name']
    #     names.append(order['name']) # should append order names
...
def main():
    ...
    # return('the total for your order is: ', total)
